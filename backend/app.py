...
class VectorStoreRetriever:
    ...
    @classmethod
    def from_docs(cls, docs, embedding_client):
        texts = [doc["page_content"] for doc in docs]
        vectors = embedding_client.embed_documents(texts)
        return cls(docs, vectors, embedding_client)

    def query(self, query: str, k: int = 5) -> list[dict]:
        query_vector = self._client.embed_query(query)
        scores = np.dot(query_vector, self._arr.T)
...
# Custom tools (you probably already have these defined elsewhere)
# from your_module import fetch_user_flight_information, ..., cancel_excursion
...
# Tool binding still works the same
